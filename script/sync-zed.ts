...
const EXTENSION_NAME = "dvpnyxcode"
...
async function main() {
  ...
  // Delete any existing branches for dvpnyxcode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating DVPNYXcode extension to v${cleanVersion}"`
