...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with dvpnyxcode...`)
  const prompt = [
  ...
  try {
    await $`dvpnyxcode run -m dvpnyxcode/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  dvpnyxcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with dvpnyxcode")
  return true
}
