...
import { $ } from "bun"
import { createDVPNYXcode } from "@dvpnyxcode/sdk/v2"
import { parseArgs } from "util"
import { Script } from "@dvpnyxcode/script"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/anomalyco/dvpnyxcode/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/anomalyco/dvpnyxcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/dvpnyxcode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/dvpnyxcode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/dvpnyxcode/")) areas.add("core")
...
async function summarizeCommit(dvpnyxcode: Awaited<ReturnType<typeof createDVPNYXcode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await dvpnyxcode.client.session.create()
  const result = await dvpnyxcode.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "dvpnyxcode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], dvpnyxcode: Awaited<ReturnType<typeof createDVPNYXcode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(dvpnyxcode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/anomalyco/dvpnyxcode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const dvpnyxcode = await createDVPNYXcode({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, dvpnyxcode)
    notes.push(...lines)
