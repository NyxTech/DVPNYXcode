...
const CLI_INSTALL_DIR: &str = ".dvpnyxcode/bin";
const CLI_BINARY_NAME: &str = "dvpnyxcode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("dvpnyxcode-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "DVPNYXCODE_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "DVPNYXCODE_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("DVPNYXCODE_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.dvpnyxcode/bin/dvpnyxcode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://dvpnyxcode.ai/install | bash -s -- --version {} --no-modify-path",
            ...
            let mut env_prefix = vec![
                "DVPNYXCODE_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "DVPNYXCODE_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "DVPNYXCODE_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "DVPNYXCODE_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "DVPNYXCODE_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "DVPNYXCODE_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("DVPNYXCODE_SERVER_USERNAME", "dvpnyxcode".to_string()),
        ("DVPNYXCODE_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn dvpnyxcode");
...
mod tests {
    ...
    fn merge_shell_env_keeps_explicit_overrides() {
        ...
        assert_eq!(merged.get("DVPNYXCODE_CLIENT"), Some(&"desktop".to_string()));
    }
