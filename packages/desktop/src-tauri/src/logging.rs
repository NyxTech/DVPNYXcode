...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("dvpnyxcode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("dvpnyxcode_lib=debug,dvpnyxcode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("dvpnyxcode_lib=info,dvpnyxcode_desktop=info,sidecar=info")
        }
    });
