...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %dvpnyxcode_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn DVPNYXcode Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
async fn setup_server_connection(app: AppHandle) -> ServerConnection {
    ...
    ServerConnection::CLI {
        url: local_url,
        username: Some("dvpnyxcode".to_string()),
...
fn get_sidecar_port() -> u32 {
    option_env!("DVPNYXCODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("DVPNYXCODE_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = dvpnyxcode_db_path() else {
        return true;
    };
...
fn dvpnyxcode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("dvpnyxcode").join("dvpnyxcode.db"))
}
