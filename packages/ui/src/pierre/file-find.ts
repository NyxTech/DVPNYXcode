...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("dvpnyxcode-find")
  api.delete("dvpnyxcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: FileFindHit[], currentIndex: number) => {
    ...
    if (active) api.set("dvpnyxcode-find-current", new Highlight(active))

    const rest = ranges.flatMap((hit, i) => (i === currentIndex ? [] : [hit.range]))
    if (rest.length > 0) api.set("dvpnyxcode-find", new Highlight(...rest))
    return true
  }
