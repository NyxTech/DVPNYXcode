...
import { fn } from "@dvpnyxcode/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "dvpnyxcode"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.DVPNYXCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            await Filesystem.write(path.join(dotgit, "dvpnyxcode"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.DVPNYXCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
