...
const TUI_SCHEMA_URL = "https://dvpnyxcode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dvpnyxcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const dvpnyxcode = await dvpnyxcodeFiles(input)
  for (const file of dvpnyxcode) {
...
async function dvpnyxcodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.DVPNYXCODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("dvpnyxcode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "dvpnyxcode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "dvpnyxcode"))
  }
  if (Flag.DVPNYXCODE_CONFIG) files.push(Flag.DVPNYXCODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "dvpnyxcode"))
