...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.DVPNYXCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.DVPNYXCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.DVPNYXCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".dvpnyxcode") && dir !== Flag.DVPNYXCODE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old dvpnyxcode.json shape) still get their settings applied.
    const normalized = (() => {
