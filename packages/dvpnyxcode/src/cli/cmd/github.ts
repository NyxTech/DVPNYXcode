...
const AGENT_USERNAME = "dvpnyxcode-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/dvpnyxcode.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://dvpnyxcode.ai/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/dvpnyxcode-agent"
            const command =
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.dvpnyxcode.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  dvpnyxcode:
    if: |
      ...
      contains(github.event.comment.body, ' /dvpnyxcode') ||
      startsWith(github.event.comment.body, '/dvpnyxcode')
    ...
    steps:
      ...
      - name: Run dvpnyxcode
        uses: anomalyco/dvpnyxcode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.dvpnyxcode.ai" : "https://dvpnyxcode.ai"
      ...
      try {
        ...
        console.log("dvpnyxcode session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.dvpnyxcode.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/dvpnyxcode,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to dvpnyxcode...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("dvpnyxcode-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `dvpnyxcode/${type}-${hex}-${timestamp}`
        }
        return `dvpnyxcode/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/dvpnyxcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[dvpnyxcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the dvpnyxcode infrastructure after your response",
