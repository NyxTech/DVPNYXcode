...
import type { Hooks } from "@dvpnyxcode/plugin"
...
export const AuthLoginCommand = cmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "dvpnyxcode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (args.url) {
          ...
          const wellknown = await fetch(`${url}/.well-known/dvpnyxcode`).then((x) => x.json() as any)
        ...
        if (provider === "other") {
          ...
          prompts.log.warn(
            `This only stores a credential for ${provider} - you will need configure it in dvpnyxcode.json, check the docs for examples.`,
          )
        }
        ...
        if (provider === "dvpnyxcode") {
          prompts.log.info("Create an api key at https://dvpnyxcode.ai/auth")
        }
        ...
        if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
          prompts.log.info(
            "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://dvpnyxcode.ai/docs/providers/#cloudflare-ai-gateway",
          )
        }
