...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run dvpnyxcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for dvpnyxcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found dvpnyxcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["dvpnyxcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting dvpnyxcode...")
        UI.println()

        // Launch dvpnyxcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const dvpnyxcodeArgs = sessionId ? ["-s", sessionId] : []
        const dvpnyxcodeProcess = spawn("dvpnyxcode", dvpnyxcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          dvpnyxcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`dvpnyxcode exited with code ${code}`))
          })
          dvpnyxcodeProcess.on("error", reject)
        })
      },
    })
  },
