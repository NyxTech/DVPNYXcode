    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: dvpnyxcode mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in dvpnyxcode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .dvpnyxcode/ subdirectory too)
  const candidates = [path.join(baseDir, "dvpnyxcode.json"), path.join(baseDir, "dvpnyxcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".dvpnyxcode", "dvpnyxcode.json"), path.join(baseDir, ".dvpnyxcode", "dvpnyxcode.jsonc"))
  }
  ...
  // Default to dvpnyxcode.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., dvpnyxcode x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "dvpnyxcode-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
