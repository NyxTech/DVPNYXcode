...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall dvpnyxcode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall DVPNYXcode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g dvpnyxcode-ai",
      pnpm: "pnpm uninstall -g dvpnyxcode-ai",
      bun: "bun remove -g dvpnyxcode-ai",
      yarn: "yarn global remove dvpnyxcode-ai",
      brew: "brew uninstall dvpnyxcode",
      choco: "choco uninstall dvpnyxcode",
      scoop: "scoop uninstall dvpnyxcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "dvpnyxcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using DVPNYXcode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# dvpnyxcode") || content.includes(".dvpnyxcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".dvpnyxcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".dvpnyxcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".dvpnyxcode"))
    ) {
      continue
    }
