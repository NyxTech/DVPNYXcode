...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at dvpnyxcode.ai",
  ...
  "Create {highlight}dvpnyxcode.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/dvpnyxcode/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "DVPNYXcode auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.dvpnyxcode/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.dvpnyxcode/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "DVPNYXcode auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "DVPNYXcode uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.dvpnyxcode/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.dvpnyxcode/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent DVPNYXcode from reading sensitive files",
  "Use {highlight}dvpnyxcode run{/highlight} for non-interactive scripting",
  "Use {highlight}dvpnyxcode --continue{/highlight} to resume the last session",
  "Use {highlight}dvpnyxcode run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}dvpnyxcode serve{/highlight} for headless API access to DVPNYXcode",
  "Use {highlight}dvpnyxcode run --attach{/highlight} to connect to a running server",
  "Run {highlight}dvpnyxcode upgrade{/highlight} to update to the latest version",
  "Run {highlight}dvpnyxcode auth list{/highlight} to see all configured providers",
  "Run {highlight}dvpnyxcode agent create{/highlight} for guided agent creation",
  "Use {highlight}/dvpnyxcode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}dvpnyxcode github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/dvpnyxcode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.dvpnyxcode/themes/{/highlight} directory",
  ...
  "Run {highlight}dvpnyxcode debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/dvpnyxcode{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with DVPNYXcode Zen for curated, tested models",
