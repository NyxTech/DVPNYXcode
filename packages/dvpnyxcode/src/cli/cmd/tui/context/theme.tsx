...
import dvpnyxcode from "./theme/dvpnyxcode.json" with { type: "json" }
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (config.theme ?? kv.get("theme", "dvpnyxcode")) as string,
      ready: false,
    })
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.dvpnyxcode, store.mode)
    })
