...
import { Module } from "@dvpnyxcode/util/module"
...
export namespace LSPServer {
    ...
    async spawn(root) {
      ...
      if (!binary) {
        ...
        if (!(await Filesystem.exists(js))) {
          if (Flag.DVPNYXCODE_DISABLE_LSP_DOWNLOAD) return
  ...
  export const Ty: Info = {
    ...
    async spawn(root) {
      if (!Flag.DVPNYXCODE_EXPERIMENTAL_LSP_TY) {
        return undefined
      }
  ...
  export const JDTLS: Info = {
    ...
    async spawn(root) {
      ...
      const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "dvpnyxcode-jdtls-data"))
      return {
        process: spawn(
          java,
          [
