...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for dvpnyxcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal dvpnyxcode sessions
...
dvpnyxcode acp --cwd /path/to/project
```
...
```bash
DVPNYXCODE_ENABLE_QUESTION_TOOL=1 dvpnyxcode acp
```
...
- **Terminal Integration**: Full terminal support via dvpnyxcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | dvpnyxcode acp
```
...
### Mapping to DVPNYXcode

ACP sessions map cleanly to dvpnyxcode's internal session model:
