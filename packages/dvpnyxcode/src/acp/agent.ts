...
import type { AssistantMessage, Event, DVPNYXcodeClient, SessionMessageResponse, ToolPart } from "@dvpnyxcode/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: DVPNYXcodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `dvpnyxcode auth login` in the terminal",
        name: "Login with dvpnyxcode",
        id: "dvpnyxcode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // DVPNYXcode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const dvpnyxcodeProvider = providers.find((p) => p.id === "dvpnyxcode")
    if (dvpnyxcodeProvider) {
      if (dvpnyxcodeProvider.models["big-pickle"]) {
        return { providerID: "dvpnyxcode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(dvpnyxcodeProvider.models))
      if (best) {
        return {
