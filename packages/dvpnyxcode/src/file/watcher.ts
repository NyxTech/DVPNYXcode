...
declare const DVPNYXCODE_LIBC: string | undefined
...
export namespace FileWatcher {
  ...
  const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
    try {
      const binding = require(
        `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${DVPNYXCODE_LIBC || "glibc"}` : ""}`,
      )
  ...
  const state = Instance.state(
    async () => {
      ...
      if (Flag.DVPNYXCODE_EXPERIMENTAL_FILEWATCHER) {
  ...
  export function init() {
    if (Flag.DVPNYXCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) {
      return
    }
    state()
  }
