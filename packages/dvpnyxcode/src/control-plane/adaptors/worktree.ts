...
export const WorktreeAdaptor: Adaptor = {
  ...
  async fetch(info, input: RequestInfo | URL, init?: RequestInit) {
    ...
    const url = input instanceof Request || input instanceof URL ? input : new URL(input, "http://dvpnyxcode.internal")
    const headers = new Headers(init?.headers ?? (input instanceof Request ? input.headers : undefined))
    headers.set("x-dvpnyxcode-directory", config.directory)
