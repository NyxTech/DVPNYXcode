...
export namespace ProviderError {
  ...
  function error(providerID: string, error: APICallError) {
    if (providerID.includes("github-copilot") && error.statusCode === 403) {
      return "Please reauthenticate with the copilot provider to ensure your credentials work properly with DVPNYXcode."
    }
  ...
  function message(providerID: string, e: APICallError) {
    return iife(() => {
      ...
      if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
        if (e.statusCode === 401) {
          return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `dvpnyxcode auth login <your provider URL>` to re-authenticate."
        }
        if (e.statusCode === 403) {
