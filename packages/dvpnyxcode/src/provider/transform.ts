...
export namespace ProviderTransform {
  export const OUTPUT_TOKEN_MAX = Flag.DVPNYXCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "dvpnyxcode" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("dvpnyxcode")) {
        result["promptCacheKey"] = input.sessionID
