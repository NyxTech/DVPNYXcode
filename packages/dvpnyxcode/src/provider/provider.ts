...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async dvpnyxcode(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["dvpnyxcode"]?.options?.apiKey) return true
        return false
      })()
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.DVPNYXCODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("dvpnyxcode")) {
        priority = ["gpt-5-nano"]
      }
  ...
  const priority = ["big-pickle", "gpt-5", "claude-sonnet-4", "gemini-3-pro"]
  ...
  export async function defaultModel() {
    ...
    // Try to find amazon-bedrock first
    const bedrock = providers["amazon-bedrock"]
    if (bedrock) {
      const [model] = sort(Object.values(bedrock.models))
      if (model) return { providerID: "amazon-bedrock", modelID: model.id }
    }
