...
function globalFiles() {
  ...
  if (Flag.DVPNYXCODE_CONFIG_DIR) {
    files.push(path.join(Flag.DVPNYXCODE_CONFIG_DIR, "AGENTS.md"))
  }
  files.push(path.join(Global.Path.config, "AGENTS.md"))
  if (!Flag.DVPNYXCODE_DISABLE_CLAUDE_CODE_PROMPT) {
...
async function resolveRelative(instruction: string): Promise<string[]> {
  ...
  if (!Flag.DVPNYXCODE_CONFIG_DIR) {
    log.warn(
      `Skipping relative instruction "${instruction}" - no DVPNYXCODE_CONFIG_DIR set while project config is disabled`,
    )
    return []
  }
  return Filesystem.globUp(instruction, Flag.DVPNYXCODE_CONFIG_DIR, Flag.DVPNYXCODE_CONFIG_DIR).catch(() => [])
}
