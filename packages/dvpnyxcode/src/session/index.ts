import { Slug } from "@dvpnyxcode/util/slug"
...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.DVPNYXCODE_AUTO_SHARE || cfg.share === "auto"))
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".dvpnyxcode", "plans")
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like DVPNYXcode's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
