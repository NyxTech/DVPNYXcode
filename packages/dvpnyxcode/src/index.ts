...
import { NamedError, ResolveMessage } from "@dvpnyxcode/util/error"
...
let cli = yargs(hideBin(process.argv))
  ...
  .middleware(async (opts) => {
    ...
    Log.Default.info("dvpnyxcode", {
      version: Installation.VERSION,
      args: process.argv.slice(2),
    })

    const marker = path.join(Global.Path.data, "dvpnyxcode.db")
