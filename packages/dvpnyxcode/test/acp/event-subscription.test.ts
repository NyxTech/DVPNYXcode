...
import type { Event, EventMessagePartUpdated, ToolStatePending, ToolStateRunning } from "@dvpnyxcode/sdk/v2"
...
function createFakeAgent() {
  ...
  const agent = new ACP.Agent(connection, {
    sdk,
    defaultModel: { providerID: "dvpnyxcode", modelID: "big-pickle" },
  } as any)
  ...
  test("routes message.part.delta by the event sessionID (no cross-session pollution)", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const cwd = "/tmp/dvpnyxcode-acp-test"
