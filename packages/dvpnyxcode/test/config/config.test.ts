...
const managedConfigDir = process.env.DVPNYXCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "dvpnyxcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "dvpnyxcode.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://dvpnyxcode.ai/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in dvpnyxcode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://dvpnyxcode.ai/config.json",
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "dvpnyxcode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://dvpnyxcode.ai/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "dvpnyxcode.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "dvpnyxcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .dvpnyxcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })
      const agentDir = path.join(dvpnyxcodeDir, "agent")
...
test("loads agents from .dvpnyxcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })

      const agentsDir = path.join(dvpnyxcodeDir, "agents")
...
test("loads commands from .dvpnyxcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })

      const commandDir = path.join(dvpnyxcodeDir, "command")
...
test("loads commands from .dvpnyxcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })

      const commandsDir = path.join(dvpnyxcodeDir, "commands")
...
test("does not try to install dependencies in read-only DVPNYXCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.DVPNYXCODE_CONFIG_DIR
  process.env.DVPNYXCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.DVPNYXCODE_CONFIG_DIR
    else process.env.DVPNYXCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable DVPNYXCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "dvpnyxcode.json"),
        JSON.stringify({ $schema: "https://dvpnyxcode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "dvpnyxcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .dvpnyxcode config
      const projectDir = path.join(dir, "project")
      const dvpnyxcodeDir = path.join(projectDir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })
      ...
      // Local .dvpnyxcode config with different plugins
      await Filesystem.write(
        path.join(dvpnyxcodeDir, "dvpnyxcode.json"),
        JSON.stringify({
          $schema: "https://dvpnyxcode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .dvpnyxcode config with some overlapping plugins
      await Filesystem.write(
        path.join(dvpnyxcodeDir, "dvpnyxcode.json"),
        JSON.stringify({
          $schema: "https://dvpnyxcode.ai/config.json",
...
// Note: preload.ts sets DVPNYXCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .dvpnyxcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .dvpnyxcode directory config enables it
      const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode")
      await fs.mkdir(dvpnyxcodeDir, { recursive: true })
      await Filesystem.write(
        path.join(dvpnyxcodeDir, "dvpnyxcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/dvpnyxcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/dvpnyxcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-dvpnyxcode@2.4.3")).toBe("oh-my-dvpnyxcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@dvpnyxcode/plugin@2.0.0")).toBe("@dvpnyxcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-dvpnyxcode@2.4.3", "file:///project/.dvpnyxcode/plugin/oh-my-dvpnyxcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.dvpnyxcode/plugin/oh-my-dvpnyxcode.js")
  })
  ...
  test("local plugin directory overrides global dvpnyxcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const dvpnyxcodeDir = path.join(projectDir, ".dvpnyxcode")
        const pluginDir = path.join(dvpnyxcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("DVPNYXCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"]
    process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .dvpnyxcode/ directories when flag is set", async () => {
    const originalEnv = process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"]
    process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .dvpnyxcode directory with a command
          const dvpnyxcodeDir = path.join(dir, ".dvpnyxcode", "command")
          await fs.mkdir(dvpnyxcodeDir, { recursive: true })
          await Filesystem.write(path.join(dvpnyxcodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .dvpnyxcode should NOT be in directories list
          const hasProjectDVPNYXcode = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectDVPNYXcode).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DVPNYXCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["DVPNYXCODE_CONFIG_DIR"]
      process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["DVPNYXCODE_CONFIG_DIR"]
      } else {
        process.env["DVPNYXCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("DVPNYXCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["DVPNYXCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["DVPNYXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["DVPNYXCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from DVPNYXCODE_CONFIG_DIR, not project
...
describe("DVPNYXCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in DVPNYXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DVPNYXCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["DVPNYXCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://dvpnyxcode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["DVPNYXCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["DVPNYXCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in DVPNYXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["DVPNYXCODE_CONFIG_CONTENT"]
