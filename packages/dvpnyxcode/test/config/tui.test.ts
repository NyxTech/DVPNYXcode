...
afterEach(async () => {
  delete process.env.DVPNYXCODE_CONFIG
  delete process.env.DVPNYXCODE_TUI_CONFIG
...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".dvpnyxcode"), { recursive: true })
      await Bun.write(
        path.join(dir, ".dvpnyxcode", "tui.json"),
...
test("migrates tui-specific keys from dvpnyxcode.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "dvpnyxcode.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(await Filesystem.exists(path.join(tmp.path, "dvpnyxcode.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when dvpnyxcode.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "dvpnyxcode.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "dvpnyxcode.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "dvpnyxcode.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "dvpnyxcode.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "dvpnyxcode.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "dvpnyxcode.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "dvpnyxcode.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple dvpnyxcode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "dvpnyxcode.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "dvpnyxcode.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over DVPNYXCODE_TUI_CONFIG (matches DVPNYXCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.DVPNYXCODE_TUI_CONFIG = custom
    },
  })
...
test("DVPNYXCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from DVPNYXCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "dvpnyxcode.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.DVPNYXCODE_CONFIG = path.join(customDir, "dvpnyxcode.json")
    },
  })
...
test("loads .dvpnyxcode/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".dvpnyxcode"), { recursive: true })
      await Bun.write(path.join(dir, ".dvpnyxcode", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
