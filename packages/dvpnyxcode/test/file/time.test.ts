...
describe("file/time", () => {
  ...
  describe("assert()", () => {
    ...
    test("skips check when DVPNYXCODE_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.DVPNYXCODE_DISABLE_FILETIME_CHECK
          ;(Flag as { DVPNYXCODE_DISABLE_FILETIME_CHECK: boolean }).DVPNYXCODE_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { DVPNYXCODE_DISABLE_FILETIME_CHECK: boolean }).DVPNYXCODE_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
