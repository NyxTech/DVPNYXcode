...
test("plan agent denies edits except .dvpnyxcode/plans/*", async () => {
  await using tmp = await tmpdir()
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      ...
      expect(PermissionNext.evaluate("edit", ".dvpnyxcode/plans/foo.md", plan!.permission).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".dvpnyxcode", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await Agent.get("build")
        const skillDir = path.join(tmp.path, ".dvpnyxcode", "skill", "perm-skill")
