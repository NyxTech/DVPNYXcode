...
describe("tool.skill", () => {
  test("description lists skill location URL", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const skillDir = path.join(dir, ".dvpnyxcode", "skill", "tool-skill")
        await Bun.write(
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const skillPath = path.join(tmp.path, ".dvpnyxcode", "skill", "tool-skill", "SKILL.md")
    ...
    try {
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const dir = path.join(tmp.path, ".dvpnyxcode", "skill", "tool-skill")
