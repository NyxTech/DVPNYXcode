...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const dvpnyxcodeModel = {
      ...openaiModel,
      providerID: "dvpnyxcode",
      api: {
        id: "dvpnyxcode-test",
        url: "https://api.dvpnyxcode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, dvpnyxcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.dvpnyxcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.dvpnyxcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          dvpnyxcode: { itemId: "msg_dvpnyxcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              dvpnyxcode: { itemId: "msg_dvpnyxcode_part" },
    ...
    expect(result[0].providerOptions?.dvpnyxcode?.itemId).toBe("msg_dvpnyxcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.dvpnyxcode?.itemId).toBe("msg_dvpnyxcode_part")
