...
describe("InstructionPrompt.systemPaths DVPNYXCODE_CONFIG_DIR", () => {
  let originalConfigDir: string | undefined

  beforeEach(() => {
    originalConfigDir = process.env["DVPNYXCODE_CONFIG_DIR"]
  })
  ...
  test("prefers DVPNYXCODE_CONFIG_DIR AGENTS.md over global when both exist", async () => {
    ...
    process.env["DVPNYXCODE_CONFIG_DIR"] = profileTmp.path
  ...
  test("falls back to global AGENTS.md when DVPNYXCODE_CONFIG_DIR has no AGENTS.md", async () => {
    await using profileTmp = await tmpdir()
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
  ...
  test("uses global AGENTS.md when DVPNYXCODE_CONFIG_DIR is not set", async () => {
    await using globalTmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "AGENTS.md"), "# Global Instructions")
      },
