...
Replace runtime Bun shell template-tag usage in `packages/dvpnyxcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/dvpnyxcode/src` is removed except:
