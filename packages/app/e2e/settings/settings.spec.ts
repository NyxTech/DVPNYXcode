...
test("changing language updates settings labels", async ({ page, gotoSession }) => {
  await page.addInitScript(() => {
    localStorage.setItem("dvpnyxcode.global.dat:language", JSON.stringify({ locale: "en" }))
  })
...
test("changing theme persists in localStorage", async ({ page, gotoSession }) => {
  ...
  const storedThemeId = await page.evaluate(() => {
    return localStorage.getItem("dvpnyxcode-theme-id")
  })
...
test("legacy oc-1 theme migrates to oc-2", async ({ page, gotoSession }) => {
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("dvpnyxcode-theme-css-light")
      })
    })
    .toBeNull()
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("dvpnyxcode-theme-css-dark")
      })
    })
    .toBeNull()
})
