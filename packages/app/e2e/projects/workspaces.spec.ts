import { base64Decode } from "@dvpnyxcode/util/encode"
...
test("non-git projects keep workspace mode disabled", async ({ page, withProject }) => {
  ...
  const nonGit = await fs.mkdtemp(path.join(os.tmpdir(), "dvpnyxcode-e2e-project-nongit-"))
  ...
  try {
    await withProject(async () => {
      ...
      expect(path.basename(activeDir)).toContain("dvpnyxcode-e2e-project-nongit-")
