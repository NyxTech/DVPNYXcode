...
class MemoryStorage implements Storage {
  ...
  getItem(key: string) {
    ...
    if (key.startsWith("dvpnyxcode.throw")) throw new Error("storage get failed")
  ...
  setItem(key: string, value: string) {
    ...
    if (key.startsWith("dvpnyxcode.quota")) throw new DOMException("quota", "QuotaExceededError")
    if (key.startsWith("dvpnyxcode.throw")) throw new Error("storage set failed")
  ...
  removeItem(key: string) {
    ...
    if (key.startsWith("dvpnyxcode.throw")) throw new Error("storage remove failed")
...
describe("persist localStorage resilience", () => {
  test("does not cache values as persisted when quota write and eviction fail", () => {
    const storageApi = persistTesting.localStorageWithPrefix("dvpnyxcode.quota.scope")
    storageApi.setItem("value", '{"value":1}')

    expect(storage.getItem("dvpnyxcode.quota.scope:value")).toBeNull()
    expect(storageApi.getItem("value")).toBeNull()
  })

  test("disables only the failing scope when storage throws", () => {
    const bad = persistTesting.localStorageWithPrefix("dvpnyxcode.throw.scope")
    ...
    const healthy = persistTesting.localStorageWithPrefix("dvpnyxcode.safe.scope")
    healthy.setItem("value", '{"value":3}')
    expect(storage.getItem("dvpnyxcode.safe.scope:value")).toBe('{"value":3}')
  })

  test("failing fallback scope does not poison direct storage scope", () => {
    const broken = persistTesting.localStorageWithPrefix("dvpnyxcode.throw.scope2")
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    expect(result).toStartWith("dvpnyxcode.workspace.")
