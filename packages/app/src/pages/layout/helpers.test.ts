...
import { type Session } from "@dvpnyxcode/sdk/v2/client"
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("dvpnyxcode://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("dvpnyxcode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("dvpnyxcode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("dvpnyxcode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("dvpnyxcode://open-project")).toBeUndefined()
    expect(parseDeepLink("dvpnyxcode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "dvpnyxcode://open-project?directory=/a",
      "dvpnyxcode://other?directory=/b",
      "dvpnyxcode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("dvpnyxcode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("dvpnyxcode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("dvpnyxcode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("dvpnyxcode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "dvpnyxcode://new-session?directory=/a",
      "dvpnyxcode://open-project?directory=/b",
      "dvpnyxcode://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DVPNYXCODE__: {
        deepLinks: ["dvpnyxcode://open-project?directory=/a"],
      },
    } as unknown as Window & { __DVPNYXCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["dvpnyxcode://open-project?directory=/a"])
